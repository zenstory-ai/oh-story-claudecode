...
- CI 新增作者汇报模板守卫：标为 `author-report` 的模板块里不得出现脚本名、字段名、参数、状态码、内部清单名或不带故事标签的编号。
...
- 作者记忆拆成项目级与书级两个 store，记忆随书走（#437，关闭 #435）：全局、题材、流程条目留在工作区 `.story/作者记忆/`（`AP`），本书条目改存书目录 `.story/作者记忆/`（`BP`），`query` 合并两级返回。书级读写须传 `--book-root`；升级前写在工作区的本书条目用新增的 `migrate --book-root` 一次性搬进书目录。
...
- OpenCode 适配改为只支持 2.x（#443，修复 #440、#441）：插件改用 2.x API 并按会话所在项目定位书目；agents 改用原生 `permissions:` 规则列表；不再写 `opencode.json`；story-setup 部署前检查 `opencode --version`，1.x 或版本读不出时停止部署。委派说明改为 2.x 的 `subagent` 工具＋`agent` 参数。
- 短篇写作按阶段读取流程（#418）：Phase 3 前读 `workflow-draft.md`，Phase 4 前读 `workflow-revision.md`。
...
- 作者记忆在单书布局（书根就是工作区）下两级 store 撞到同一个文件，`query` / `migrate` / 书级 `record` 全部失败、连全局偏好也读不到。现在书根与工作区相同时书级 store 住 `.story/作者记忆/书级/`；已被写坏的工作区在下一次带 `--book-root` 运行任一命令时自动恢复，不丢条目。
- 作者记忆写入成功后另一级 store 读不出来时，不再返回失败让 agent 误报「没记住」、换 `event_id` 重试造成重复条目。
- 短篇写前参数验收在 #418 拆分后失效（实测先写正文、事后才验收）：Phase 3 细则现在自带完整验收命令并排在动笔前，写作中改用交付验收脚本计数，新增回归守卫。
- 长篇写手的分组片段、prompt 留档与每章追踪事务文件统一放在书目录 `.story/work/第NNN章/`，不再写到系统 `/tmp`（多本书同章号会互相覆盖，Windows 上也没有）或 `正文/`、`大纲/`；每章提交成功后自动清理。
- v0.7.10 的半截拆文库遇到以楔子/序章开头的书会静默错位一章（楔子没人拆、第三章拆两遍）：现在按旧「章节边界」表逐章核对，对不上就在写索引前停下，用作者的话给出三个选择。拆文脚本在 Windows 英文 / GBK 控制台下不再崩溃或乱码。
- 人物关系图在没有中文字体的环境里不再退回拼音首字母。
- OpenCode 1.x 或版本不明时仍会部署，新写法的 `permissions:` 在 1.x 下被忽略，三个只读 agent 实际能写文件、跑命令；被拦下时也不再写或升级 `.story-deployed`。story-review 不再按旧的 `permission` 字段检查 OpenCode 2.x agent 而误判降级。
- 写正文拦截提示给出补零的细纲文件名（`细纲_第001章.md`）；写入路径含未展开的 shell 变量时，改为提示使用字面路径，不再误报「缺少细纲」。

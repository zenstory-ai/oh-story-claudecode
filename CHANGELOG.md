...
- CI 新增作者汇报模板守卫：标为 `author-report` 的模板块里不得出现脚本名、字段名、参数、状态码、内部清单名或不带故事标签的编号（编号写成「描述（ID）」或「ID（描述）」）。
...
- 长篇写手的分组片段、prompt 留档与每章追踪事务文件统一放在书目录 `.story/work/第NNN章/`，不再写到系统 `/tmp`（多本书同章号会互相覆盖，Windows 上也没有）或 `正文/`、`大纲/`；每章提交成功后自动清理。
- v0.7.10 的半截拆文库遇到以楔子/序章开头的书会静默错位一章（楔子没人拆、第三章拆两遍）：现在按旧「章节边界」表逐章核对，对不上就在写索引前停下，用作者的话给出选择（开头有楔子时推荐并入第一章，原文更新后重建自动沿用上次的并入口径）；旧表标题有删减或加注（如「（求收藏）」）但起始行对得上时照常续拆。拆文脚本在 Windows 英文 / GBK 控制台下不再崩溃或乱码。

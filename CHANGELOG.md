...
- 长篇拆文 Stage 2 主会话不再中转内容：chapter-extractor 按行号自读原文，把整批结果写进 `_analysis_cache/输入-{批次ID}.md`，只回一行回执；主会话不读这份文件，直接提交，被拒时交回子代理重写。chapter-extractor 因此有了写文件权限，写入路径由内联 hook 限定在这一类文件。派发方式分串行 / 有限并行（每轮 3 批）/ 不限批次顺序（同时最多 12 个子代理、不超过环境上限，不足一半补满，出错减半）三档，Stage 1 后停下询问时由作者选；没选、全量拆、多本同拆或导入自动续跑都按不限顺序；已确认别名随跨批状态传递。

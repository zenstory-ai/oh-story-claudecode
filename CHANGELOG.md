...
- 作者给定的字数范围之前不生效，一律按默认 ±15%；现在写进细纲「字数范围」行，前组测量与整章检查都按它算（命令参数只作临时覆盖）。
- 旧细纲里「字数目标：按卷规划」这类不带数字的说明行会让章节检查和已提交章的修订报错；现在不带数字的行当说明忽略，多个数字互相矛盾才报错。
- 正文里的 HTML 注释：写后检查会报注释里的句式、指去的章节检查却说干净，模型原地打转；两边现在都跳过注释。
- 检测脚本或标点脚本崩溃、输出读不出来时，章节检查曾把本章当作通过；现在报「工具不可用」，不能提交。
- GBK 编码的细纲在 Claude Code 与其他端的空细纲判断不一致，现在各端一致。
...
- **续写章不再系统性写短**：Claude Code 上的写手改用 Opus。同一批真实写作任务里，Sonnet 写手续写章平均只到目标字数的 71%，三章只有一章首检在范围内；Opus 平均 107%、三章都在范围内，异家族评委配对盲评三章全胜；按 API 计价每章成本基本持平，用订阅时 Opus 消耗额度比 Sonnet 快。只改篇幅说明或让写手自测补写都没有效果。证据来自同一本书的三章对照，完整日更复测三章首检都在范围内，更多题材仍在观察。
...
- **章节检查的结果更明确**：顶层 `status`（可提交 / 字数出带待定 / 有必须修问题 / 正文无效 / 工具不可用）与退出码一一对应；缺 Node 时报「工具不可用」而不是伪装成正文问题；按当前长度收下设了下限（低于目标一半，或超长未压缩过一次，需要作者明确坚持），被拒时主会话按原因告诉作者差多少、该补还是该压。
...
- `tracking_commit.py draft`：按当前追踪状态预填一章的追踪提交（修订号、章名、当前上下文、各字段字数上限、在场角色当前快照），只需填本章变化和本章结尾的故事时间、场景；草稿已存在时重跑只刷新修订号。
...
- **概念收敛**：
  ...
  - 问题严重度统一读作 必须修 / 建议看 / 仅提示（S1/S2 都算必须修）；给作者的审查报告照旧写 必须改 / 建议改 / 可以不改。
  - 去 AI 味写明只有一条管线（检测器 → 选 Gate → 一次定点改写 → 复扫），三遍法不再作为独立层级。
  - 去掉 Constraint Lock、「内带」两个名词，行为不变。
  - 细纲必填收到 9 个核心字段（核心事件、字数目标、字数口径、单元位置、目标情绪、主角目标、章首钩子、爽点、本章禁止提前释放）；阶段位置、章节定位、结构公式、本章标价、闭环状态、写手自由区、镜头准入、信息差触发点、分辨率列改为需要时再写，情节点表默认四列「禁＋放」。
  - 写手不再交时空表、不再默认把同一场戏的情节点交错排布（对照实验测不出收益），交付改为一行一点的「情节点落点」；对话自检并入 dialogue-mastery；质检侧删除测试收成「套式反应删除测试」与「写法抽查」两块。
  - 旧信息查找由 6 级收成 3 级；写手「本章没写成的」只分「细纲没给 / 要先问作者」两路；改稿后一次 `chapter check` 取代分别调检测与整读禁用词表。
- narrative-writer 写新正文不再预加载去 AI 味整套流程、不预读禁用词表，也不自己跑检测脚本；对话与情绪弧线参考照常读。
- **长篇拆文主会话不中转内容**（#446）：
  - 长篇拆文 Stage 2 主会话不再中转内容：chapter-extractor 按行号自读原文，把整批结果写进 `_analysis_cache/输入-{批次ID}.md`，只回一行回执；主会话不读这份文件，直接提交，被拒时交回子代理重写。chapter-extractor 因此有了写文件权限，写入路径由内联 hook 限定在这一类文件。派发方式分串行 / 有限并行（每轮 3 批）/ 不限批次顺序（同时最多 12 个子代理、不超过环境上限，不足一半补满，出错减半）三档，Stage 1 后停下询问时由作者选；没选、全量拆、多本同拆或导入自动续跑都按有限并行；已确认别名随跨批状态传递。
  - chapter-extractor 输出前检查新增机械自查：写完用 Grep 计数核对情节点标题行、主题标签｜基调行和十个章级字段，对不上先用 Edit 就地补齐再回执；提交被拒时同样只改出错处，不整份重写，减少重跑的输出量。
  - `manage_analysis_run.py plan` 新增 `--next N`：只输出前 N 批（`0` 只看进度），总数 `remaining_batches` 照给；逐批派发改用 `--next 1`，不再反复读全量计划。
  - Stage 3–5 改用只读的 `manage_analysis_run.py digest` 取料（全书跨章观察，外加按章节窗口抽取的逐章字段与情节点简表），不再整份读取批次缓存。
  - 情节点密度规则区分原文块与旧成果补摘要；逐章「涉及人物」与角色建档排除群体称呼、无名路人与泛称。
...
- 写正文守卫漏拦 `cd 正文 && cat > 第N章.md` 这类先进正文目录再写的命令。
- 追踪文档与单章流程关于「写正文时是否在章中测字数」的说法互相矛盾。
- 测试审计：清理重复、测不到东西或只钉措辞的测试与守卫，补上此前测不到的引号豁免、上一章毒句式欠账门与多条检测器分支的行为用例。
...
- 长篇拆文新增可选三层灵感库管道（#444）：复用 Stage 3 的情绪机制卡，登记行进 `灵感索引.csv`，单书合并增量与跨书写作卡分层存放；说「灵感库」「跨书灵感聚合」「更新灵感库」时触发，单书拆文不自动入库。story-long-write 只在开书、卷纲、细纲三处可选召回，逐章写作不接灵感库，无库或零命中不阻塞。
- 人物关系图改由脚本从角色关系表生成，主产物是任何 Markdown 查看器都能显示中文的 `人物关系图/人物关系图.md`；只有找到覆盖全部字形的中文字体时才另出 PNG。
- CI 新增作者汇报模板守卫：用 `<!-- author-report -->` 标记的模板块里不得出现脚本名、字段名、参数、状态码、内部清单名或不带故事标签的编号（编号写成「描述（ID）」或「ID（描述）」）。

### Changed

- **给作者看的话一律改成作者视角**：长篇写作、短篇写作、拆文、导入、审稿、去 AI 味、扫榜、封面、作者记忆回执与 story-setup 安装报告，都只讲写了或改了什么（用故事的话）、需要作者拍板什么（白话问题＋推荐默认）、下一步怎么说；脚本名、字段名、状态码、S1–S4、Gate、内部清单名与裸编号移出正文，至多保留末尾一行技术备注。长篇字数不在范围时的三个选项改为「就按现在的长度收下 / 我改细纲或字数目标再重写 / 这章不要了」；审稿问题按「必须改 / 建议改 / 可以不改」分档。
- 长篇规划与正文分开授权（#420，关联 #419）：「写大纲 / 规划剧情 / 规划第X卷」只交付所请求的大纲、卷纲与必要设定，不再自动展开首批 10 章细纲；「写/修改第N章细纲」按细纲规划处理，不进入正文大修；「只讨论结构」不建工程、不落盘；「继续 / 按这个来」不扩大当前范围。刚做完规划时说「继续写」，只用一句话确认要不要写下一章正文。未限定层级的「帮我开书」仍交付到首批细纲。
- 追踪只在第一次写正文时初始化，开书、大纲、细纲阶段不再生成 `追踪/`。
- 部署到写作项目的指令模板新增「与作者协作」三条：作者说「记住 / 忘掉 / 确认」写作习惯时走 oh-story 作者记忆，不存进编程工具自带的记忆；不修改项目里已安装的 skill 文件，脚本报错就停下把报错告诉作者；回复用写书的话说。
- 长篇拆文改为单状态三脚本运行时，需要 Python 3（#438）：`_progress.md` 受管区是唯一运行状态；原文块最多 3 章、25,000 字符，每章 10–30 个情节点，越界整批拒收。完整旧项目默认直接复用、不读原文，部分项目只补缺章，已有逐章摘要永不覆盖。chapter-extractor 与 story-explorer 模板同步更新。
- 作者记忆拆成项目级与书级两个 store，记忆随书走（#437，关闭 #435）：全局、题材、流程条目留在工作区 `.story/作者记忆/`（`AP`），本书条目改存书目录 `.story/作者记忆/`（`BP`），`query` 合并两级返回。书级读写须传 `--book-root`；升级前写在工作区的本书条目不再参与查询（不做双读），用新增的 `migrate --book-root` 一次性搬进书目录。单书布局（书根就是工作区）下书级 store 住 `.story/作者记忆/书级/`，与项目级各自一份 state；从 main 部署过预发版、已被写坏的单书工作区，在下一次带 `--book-root` 运行任一命令时自动归位，不丢条目，多书工作区里把书目录误传成工作区时拒绝执行、不改文件。写入成功后另一级 store 读不出来只给提醒，不再返回失败诱发重复写入。
- 作者记忆只记作者明确表达的偏好（#437，关闭 #436）：`record` / `commit` 拒绝 `repeated_correction` / `inferred_pattern` 两个推断来源；存量旧来源条目照常可读、可确认、可退役。
- 作者记忆注入预算从读时静默截断改为写时预警（#429）：写入照常成功，装不进 2048 字节时在 `warnings` 点名将被略过的条目；查询按「重要度 → 本书例外 → 最近更新」装填；`query --kind` 改为必传。
- OpenCode 适配改为只支持 2.x（#443，修复 #440、#441）：插件改用 2.x API 并按会话所在项目定位书目；agents 改用原生 `permissions:` 规则列表；不再写 `opencode.json`；story-setup 部署前检查 `opencode --version`，1.x 或版本读不出时停止部署（1.x 不认 `permissions:`，只读 agent 会拿到写文件与 shell 权限），被拦时不写或升级 `.story-deployed`；多端部署时 OpenCode 被拦，升级后重跑会询问是否把 OpenCode 加回来。story-review 按 `permissions:` 检查 OpenCode agent。委派说明改为 2.x 的 `subagent` 工具＋`agent` 参数。
- 短篇写作按阶段读取流程（#418）：Phase 3 前读 `workflow-draft.md`，Phase 4 前读 `workflow-revision.md`。写前参数验收命令留在 Phase 3 细则里并排在动笔前，有回归守卫。
- 文档：Codex 的 hooks 在 `/hooks` 信任之前会被静默跳过（含写正文前的大纲守卫）；`agy -p` 须带 `--add-dir "$PWD"` 才加载工作区；作者记忆按两级存储说明；README 改为面向读者，部署、架构与知识体系移至 `docs/`（#421–#428、#432–#434、#442）。

### Fixed

- 长篇写手的分组片段、prompt 留档与每章追踪事务文件统一放在书目录 `.story/work/第NNN章/`，不再写到系统 `/tmp`（多本书同章号会互相覆盖，Windows 上也没有）或 `正文/`、`大纲/`；每章提交成功后自动清理。追踪初始化、作者记忆事件与长篇拆文批次的临时文件也各有项目内落点，不再写系统 `/tmp`。
- v0.7.10 的半截拆文库遇到以楔子/序章开头的书会静默错位一章（楔子没人拆、第三章拆两遍）：现在按旧「章节边界」表逐章核对，对不上就在写索引前停下，用作者的话给出选择（开头有楔子时推荐并入第一章，原文更新后重建自动沿用上次的并入口径）；旧表标题有删减或加注（如「（求收藏）」）但起始行对得上时照常续拆。拆文脚本在 Windows 英文 / GBK 控制台下不再崩溃或乱码。没有子代理时由主线程自己写批次，批次提交格式现在写在拆文 skill 自己的说明里，不用再靠报错反推。
- 人物关系图在没有中文字体的环境里不再退回拼音首字母，也不会选中只画方框的兜底字体出 PNG。
- 写正文拦截提示给出补零的细纲文件名（`细纲_第001章.md`）；写入路径含未展开的 shell 变量或命令替换（`$PROJ`、`${BOOK}`、`$(pwd)`）时仍拦截，改为提示使用字面路径，不再误报「缺少细纲」。
- 卷纲剧情单元编号模板写成 `L{卷号}-{序号}`（如 L1-03），避免被写成 `1-03` 后校验报错；chapter-extractor 旧成果模式示例与 3 章批次上限对齐。
- story-long-analyze 的描述恢复触发词（「帮我拆这本书」「分析黄金三章」等）；README 与架构文档不再宣称写后 hook 提醒字数欠账。

### Removed

- 移除误入仓库的一次性评测目录 `demo/craft-stock-reaction-eval/`、`demo/style-precedence-eval/`（#430），以及《盘龙》demo 中的第三方原文（#434）。

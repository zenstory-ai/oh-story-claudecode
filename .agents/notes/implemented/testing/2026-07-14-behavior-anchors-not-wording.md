...
- 写后毒句式网的 js↔py 同步行为与源文本各锁一半：`test-prose-net-parity.sh` A 段对各条正则有正例并锚定命中，文案随 findings 逐字 diff；Claude bash 欠账门（豁免窗口、全角冒号、拦截文案）归 D 段场景。fixture 只覆盖部分字符类备选与可选组，单端删一个备选字、改一个量词界 fixture 照样全绿（变异测试实测），所以 `check-hook-regex-sync.sh` 仍对正则本体、常量表（文末窗口长度、分句边界集、疑问尾/确认语排除集）与欠账门标记做 js↔py 源文本比对；文案不在此锁。
...
- **代价与已知上限**：行为测试更慢（`check-story-setup-deployment.sh` 超过两分钟）；文档漂移没有 CI 兜底，只靠人；源码策略守卫要靠变异测试防被骗，多一层维护。若某条行为无法在 CI 环境执行（如 cmd.exe 路径），只能退回静态保形并在注释里写明是 best-effort。毒句式正则的源文本锁要求 js 字面量与 py raw string 保持同一写法，改写法（不改语义）也得两端一起改。

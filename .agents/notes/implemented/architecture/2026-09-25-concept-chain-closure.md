...
- **复审补丁**：写后检查 hook 两种实现与检测器用同一规则跳过 HTML 注释；「字数口径」「字数目标」「字数范围」三行在 JS 与 Python 用同一写法，不带数字的目标/范围行当说明忽略；非法 UTF-8 细纲三端都按非空放行。检测器或标点脚本崩溃、输出不可解析时 `chapter check` 报 tool_unavailable（退出 3），不再当通过。`wordcount checkpoint --project` 从细纲读目标与作者范围，前组剩余区间与整章检查同口径；两个参数同给时不再解析细纲范围行。拒收（BELOW_ACCEPT_FLOOR / COMPRESSION_REQUIRED）后主会话按原因问作者，不到目标一半时「按现在长度收下」不标推荐。`sync-opencode.py` 规范化 CRLF，matcher 为 `*`、空或缺省都当覆盖写工具。OpenCode 权限通配 `*` 编译为带 s 标志的 `.*`，可跨多层目录（上游 `util/wildcard.ts`）。
- 已知未修：超长基线存在 `.story/work/`，先写长稿测一次再删回原长可以绕过「压缩过一次」的判定；需要刻意操作，暂不处理。

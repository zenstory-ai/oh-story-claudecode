...
After `$story-setup` deploys into a writing project, it creates `.codex/agents/*.toml`, `.codex/hooks.json`, `.codex/hooks/{story_codex_hook.py,run-story-hook.sh,run-story-hook.cmd}`, and `.codex/skills/story-setup/references/agent-references/`. Trust the project `.codex/` layer, review/trust hooks in `/hooks`, and open a fresh Codex session so custom agents load. **Until the hooks are trusted in `/hooks`, Codex silently skips them, including the outline-before-prose guard**, with no error or notice; `codex exec` in automation can pass `--dangerously-bypass-hook-trust`.
...
**Author preferences persist across sessions:** tell `/story` to remember a writing habit (in a deployed project, a plain 「记住：……」 is routed here too, not into the coding tool's built-in memory). Author memory has two levels: global, genre, and workflow preferences go to the workspace `.story/作者记忆/` (`AP` entries); preferences for one book go to `.story/作者记忆/` inside that book's directory (`BP` entries) and travel with the book when it is archived or moved. The write counts as successful only when it returns an `Author Memory Receipt`. Normal writing queries only relevant confirmed items with a hard 2 KB output cap, rather than injecting the full profile, candidates, and history into the prose prompt. This memory stays separate from per-book continuity tracking, and current instructions, book settings, and hard gates always take priority.
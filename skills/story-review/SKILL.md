...
若作者记忆 state 已存在，审查前用 `scripts/author_memory_commit.py query --kind delivery --kind interaction --kind prose_style --book {本书}` 获取本次相关 active 条目（`--kind` 必传；不传 `--book` 就拿不到本书级偏好；总输出 ≤2KB）。它们只能帮助解释意图和组织报告，不能降低 rubric 严重度、把事实冲突判为无问题或跳过平台门禁；当前请求仍优先。完整规则见 [references/author-memory.md](references/author-memory.md)。

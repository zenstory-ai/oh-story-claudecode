...
先调用 story-long-analyze 的「资料情况与兼容路径」只读检查，登记实际文件和覆盖范围；检查建议不能覆盖下列语义规则：
...
- **旧成果可以导入，但缺少当前写作/对标功能需要的 `剧情/节奏.md`、`剧情/情绪模块.md` 等资料**：先用现有逐章、剧情、关系、报告和文风做 Stage 3+ 按需增强；证据不足时才按机械索引定点回读原文。不得重跑已完成章节，也不得用空壳文件让门禁通过。
...
| 2 | 连续章节块提取 | 待处理连续原文、旧成果与跨块状态 | 同次产出 `章节/第N章_摘要.md`（含情节点序列）和 `_analysis_cache/批次-*.md`；通常 3-5 章/批，长章缩到 1-3 章；每章 10-20 个情节点，长章最多 30 | 可用正文覆盖完整，摘要数与可用章节数一致 |
...
- 已选外部对标缺 `剧情/节奏.md` 或 `剧情/情绪模块.md` → 报告 `module_or_rhythm_required_missing`，提示重跑 `/story-long-analyze` Stage 3+，并停止该对标召回；不得用摘要或报告生成空壳。本书核心工程迁移不回滚。两份老权威产物都存在时正常同步，缺少本次新增内嵌字段不阻断。

...
| 2 | 连续章节块提取 | 待处理连续原文、旧成果与跨块状态 | 同次产出精简 `章节/第N章_摘要.md` 和 `_analysis_cache/批次-*.md`；通常 5-10 章/批，长章缩到 1-5 章，不设每章情节点硬下限 | 可用正文覆盖完整，摘要数与可用章节数一致 |
...
- 已选外部对标缺 `剧情/节奏.md` 或 `剧情/情绪模块.md` → 当前契约产物报告 `module_or_rhythm_required_missing` 并停止该对标的新召回；检查器确认为完整旧拆文时仍同步其可用资产并标 `legacy_benchmark_limited`，原有参考能力可用，依赖新字段的能力提示按需增强。本书核心工程迁移不回滚，也不生成空壳文件。

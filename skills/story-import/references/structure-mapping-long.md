...
| 摘要字段 | 细纲字段 | 转换方式 |
|---------|---------|---------|
| 关键行动 + 局面结果 | 核心事件 | 合并为“谁采取什么行动 → 局面如何变化”；多情节点摘要缺少这两项时回退 `关键事件` |
...
| 因果 + 关键行动 + 局面结果 | 内容概括（起因/发展/转折/高潮/结尾） | 先按因果链归纳；兼容多情节点摘要时才回退情节点顺序，证据不足写 `[待补充]` |
...
| 情节点 + 情节点类型 | 情节细化 / 情节点序列 | 单情节点摘要按事实与类型写入；多情节点摘要保持原多条顺序，不反推逐点字数配额 |

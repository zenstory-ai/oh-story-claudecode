...
| 摘要字段 | 细纲字段 | 转换方式 |
|---------|---------|---------|
| 关键行动 + 局面结果 | 核心事件 | 合并为“谁采取什么行动 → 局面如何变化”；旧摘要缺少这两项时回退 `关键事件` |
...
| 因果 + 关键行动 + 局面结果 | 内容概括（起因/发展/转折/高潮/结尾） | 先按因果链归纳；兼容旧摘要时才回退情节点顺序，证据不足写 `[待补充]` |
...
| 情节点 + 情节点类型 | 情节细化 / 情节点序列 | 新投影通常只有一条关键情节点，按事实与类型写入；旧摘要有多条时保持原顺序，不反推逐点字数配额 |
...
| `拆文库/{对标书名}/章节/*.md`（第N章_摘要.md + 黄金三章 第1-3章_深度拆解.md）| `{项目}/对标/{对标书名}/章节/*.md` | 匹配章证据；摘要读取概要、关键行动、局面结果、涉及、状态变化、情节点类型、主题标签与基调，黄金三章另含深度写法 |

...
`小节大纲.md` 用 story-short-write 的固定 12 列表格，每节一行，列名和顺序不改：

```markdown
| 结构段/五段功能 | 主事件 | 情节推进 | 情绪 | 人物/关系变化 | 因果/逻辑链 | 读者新获知什么 | 结尾承接/钩子 | 伏笔/物件 | 场景形态 | 对白作用 | 目标字数 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 开头 | 从情节节点提取的主要情节点 | 具体发生了什么{发现} -> 具体发生了什么{冲突} | 前状态→后状态 | 本节前后变化，或 无（本节改变：…） | 原因 → 行动 → 结果 → 后果 | [待补充] | [待补充] | [待补充] | [待补充] | [待补充] | 按原文该节实际字数 |
```

- `结构段/五段功能` 填 开头/铺垫/升级/反转/结尾。
- `情节推进` 每个推进单元带 `{对话}` `{冲突}` `{伏笔}` `{回忆}` `{发现}` `{递进}` 之一，有先后用 `->` 连接，按原文实际发生的写，不为凑数拆分。
- 从原文判断不出的格子写 `[待补充]`，留给作者或续写时填写；不写 `{}` 模板占位。
- `目标字数` 写原文该节实际字数，各节之和与 `设定.md` 的目标字数对得上。
- 付费点在哪一节末能从原文认出来时，在该行 `结尾承接/钩子` 标「付费点」；认不出写 `[待补充]`。

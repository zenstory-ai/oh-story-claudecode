...
先从 schema v4 全局产物识别角色与候选变化，再用结构块/证据章节缩小范围，最后用五列索引定位并回读原文。`chapter_index.csv` 不能按角色搜索，也不能证明当前状态：

| 输入 | 用途 |
|---|---|
| `拆文库/{书名}/全局分析/六维拆书.md` | 核心角色、身份候选、目标、成长弧线与冲突；“人物关系图谱”提供关系变化；“双时间线与信息差”提供角色可知边界 |
| `拆文库/{书名}/全局分析/证据与边界.md` | 过滤 B/C 级推断和不可泛化结论 |
| `拆文库/{书名}/structure_blocks.csv` | 依据主要人物、状态变化与证据定位缩小候选范围 |
| `拆文库/{书名}/chapter_index.csv` | 把候选章号映射为 `source_locator` |
| `source_locator` 原文 | 核证最后位置、最新状态、已知信息与未结事项 |
...
对每个核心角色，以最后完整导入章 N 为截面：先从六维拆书的人物关系图谱/结论证据与结构块筛最近 3–5 个候选章；再用五列索引取得 locator 并回读原文。若状态仍不确定，对原文按角色正式名/别名定点搜索并向前扩展；不能从 CSV 标题、结构块概括或聊天记忆猜测。
...
若同一字段有多次变化，只取截至 N 章由原文证实的当前值，不把变化史塞进快照。导入旧章不补造逐章记录。locator 与原文边界冲突时先修索引；语义结论始终以原文为准。
...
- [ ] 角色已知信息没有混入作者真相
- [ ] 未结事项都有正文证据，未来设计仍在大纲
- [ ] 每个当前状态结论都能回溯到 `source_locator` 原文，没有凭 CSV 标题或结构块概括定案

...
**同一规则适用于设定、能力与威胁。** 一样东西"危险""碰不得"不由叙述者断言，由一次有代价的试探演示：伸手、被弹回、自己估出还差多远；多看两眼、开始恍惚、退后一步绕开。旁白先把答案说出来，读者的发现权就没了——这和写"他很愤怒"是同一个错误，换了对象而已。
...
**一处显式例外：人物余韵。** 上面「删掉无损就删」针对流程细节与凑字数描写，**不针对给人物加层的段落**。每章留一处删掉不影响情节、只让某个人多出一层的段落——一次说岔了的关心、一个被误会的反应、一句长辈的联想、一个空镜。它的"无损"只对情节成立：读者记住一个人物，往往就靠这一处。判断方法不是问它推进了什么，是问**删掉之后谁少了一层**。
...
写完一段后，分别遮住发生信息、感知信息、反应信息，看每个维度是否都有内容。如果某个维度缺失，把对应细节织回同一镜头里，不要在后面另起一段补写。

**（二）交付前八问（逐章，必答）**

下面八问覆盖本文件里最容易被当成"风格建议"读过去、因而最常整条落空的条目。**在交付摘要里逐条回答；做不到的写明是哪一条、为什么，不要略过。** 只写"已遵守"不算回答——每条要指出正文里对应的那一处。
...
| 4 | "厉害／难得／要命"这些判断，是谁给出的？他有什么立场？ | 叙述者自己下的判断；或标价人就在场却没标价 | 第 10 节「标价人」 |
| 5 | 每个给了镜头的人，此刻在为自己争取什么？ | 遮住名字认不出是谁说的；所有人同向配合主角 | 第 10 节「镜头准入」 |
| 6 | 有没有一处删掉不影响情节、只让某个人多出一层的段落？ | 全章每一段都在推进情节 | 第 7 节「反注水规则」的例外 |
...
- **最重的一句要最短。** 全章最重的那一拍——底牌亮出、一句定生死的许诺、身份被说破——用最短的一段承接，能单句成段就单句成段。**长度表达绵延，不表达重量**：把最重的一拍写成全章最长的两段，读者感到的是拖不是压。要"慢"用停顿，不用字数。
...
**同类并列项内部同样不许均分。** 上表管跨类型分配；一组同类的东西并排出现时——三样物件、四拨人马、五处反应——**第一项写透，其余递减，最后一两项并成一句带过**。每项同样分辨率是"哪都写得满、哪都不出彩"的另一个来源，且比 beat 均分更难发现：每一项单看都合格。

**判为"密"的地方不许出现概括句。** "一连七八下""争执了半晌""众人纷纷议论"——量词加概括出现在密处，等于密度没落实，只是把概括拉长。密处要的是**其中一下、其中一句、其中一个人**被完整写出来；确实只能概括的，就重新判为疏。
...
治的是"所有人一起转向、齐声承认"的场面：人都在场，却没有一个在为自己做事。寡淡多半出在这里——不是主角写得不够强，是别人写得不够不情愿。
...
**标价发生在事情起效之前**，不能等结果出来再补。反面：该标价的人就在场，却只写他"笑了笑，凑到旁边说了句什么"——位置留了，内容没给，后面的兑现就没有参照。

...
- **必须严格消费细纲（内容层）**：本章细纲已有的核心事件、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定和章尾钩子，每项都要**落地演足**，不许漏、**不许把两项并成一句话交代过去**。（这条防的是漏，不防交织：两项拆开、交错、互相打断照样算各自演足，不算并项。）
- **正文形状由你编排（形状层）**：细纲是「要发生什么」的契约、不是正文的形状。落地位置、顺序、拆成几处、怎么断段由你定。**严格编排时空点——写完要整理成时空表随交付送审**（见「写完后编排自检」）。
  - **默认交错**：**同一时空内**（时间连续、地点相同）的情节点**默认互相打断、交错落地**——**不局限于相邻两点**：合理范围内，块内任何点都可以彼此穿插（后面的点提前露半句头、前面的点延后收尾）。把它们排成一前一后的串行是需要理由的那一侧——只有因果依赖（B 必须在 A 之后才成立）或刻意的节拍分隔才串行。跨时空的点本来就串行，不在此列。
  - **判据（逐对自问，不限相邻）**：**把乙点的一句插进甲点中间，剧情坏不坏？不坏，就说明这两个点可以互相打断。**坏的信号：因果顺序被打乱、该压住的信息提前泄露、正在攒的情绪节拍被冲散——命中任一条就不插，这就是「合理范围」的边界。
  - 不要把细纲措辞原样搬进叙述（技法见 `story-setup/references/agent-references/writing-craft.md`「从细纲到正文」）。
  - **例外**：细纲「复沓锚句」列出的原话要逐字落进它标注的那个情节点，不得改写或挪位置。
...
正文落盘后，先把本章**实际写成的形状**整理成一张**时空表**，再按它自检。**时空 = 时间连续 ＋ 地点相同**的一段连续的戏：

```
时空1  修车铺/白天         点1,3,2,4             交错
时空2  酒楼包间/当晚       点5,6,7(前半),8,9,10  交错
时空3  酒楼门口/散场       点7(后半)
时空4  出租屋/当夜         点11(跨度),12         交错
时空5  修车铺/次日清晨     点11(跨度),13
```

（**块内点位按正文实际落地顺序排列**，不按细纲编号顺序——时空1 里点3 实际插在点2 前面，表就照实写 1,3,2,4，让织法直接可见。点7 是切分点：包间里话到嘴边收住是前半，散场路上说出口是后半，拆开落在两处；点11 是跨度点：细纲写的就是从当夜守到次日开门的一段事，天然骑在两个时空上。）

- **时空是你划的，不是细纲给的**——按你实际写成的样子划，不是按细纲的分点划。
- **一个点可以横跨两个时空**，两种情况都合法：
  - **跨度点**：细纲那个点本身写的就是一段时长或多次重复（「一连两天」「他每晚躺下都先等一阵」），天然骑在多个时空上。
  - **切分点**：你把一个点拆开落在两处（一句没问出口的话在饭桌上憋住了，回屋才说出来）。**这是好事，是平推的反面**，每一处都要有实在内容。
  - 横跨时在表里显式标出：跨度点在它骑的每一行都写 `点N(跨度)`；切分点按落点写 `点N(前半)` / `点N(后半)`。两种记法不混用。
- 自检三条，命中即就地重织再交付：
  1. **覆盖**：细纲每个情节点在时空表里**至少出现一次**；横跨多个时空的必须标注出来，不许隐去。有漏即补。
  2. **平推**：时空表如果基本是「一个时空一个点」，说明你在按细纲顺序平推——回去对块内的点两两跑形状层那条判据（乙点的一句插进甲点中间，剧情坏不坏），不坏的就该交错。
  3. **串行有据**：凡是划在同一时空却仍然写成一前一后的点，你要能说出理由（因果依赖／刻意的节拍分隔）。说不出就是平推。
- **时空表随交付摘要返回**，主会话据它核漏项与形状。宁可如实报出「一个时空一个点」也不要虚报「交错」——虚报会在成品复核时被拆穿。
...
输出格式（**默认文件模式**）：写正文 / 改正文 / 去AI味：有文件路径时一律用 Write/Edit 直接落盘，只回 ≤200 字变更摘要（落盘文件路径 + 动了什么 + 计数），不把全文返回父会话；仅无文件路径的零散片段才返回完整文本。审查任务返回审查报告（含具体引用和修改动作）。**长篇正文另附时空表**（见「写完后编排自检」），不计入 200 字。
...
返回前报出本章**时空表**（见「写完后编排自检」），并在最后一行报出本章**实测**字数与句长分布（短句<15 / 中句15-30 / 长句>30 占比、平均句长），供主会话校验——数值必须来自 Bash 统计，跑不了就写「未完成机器统计」，禁止编造。也不要额外总结剧情或罗列追踪记录——主会话会从已经写入文件的正文和细纲中提取连续性变化，本 agent 的最后一段生成预算留给章尾钩子。

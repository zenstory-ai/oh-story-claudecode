...
- **正文形状由你编排（形状层）**：细纲是「要发生什么」的契约、不是正文的形状。落地位置、顺序、拆成几处、怎么断段由你定。**严格编排时空点——写完要整理成时空表随交付送审**（见「写完后编排自检」）。
  - **默认交错**：**同一时空内**（时间连续、地点相同）的相邻情节点**默认互相打断、交错落地**；把它们排成一前一后的串行是需要理由的那一侧——只有因果依赖（B 必须在 A 之后才成立）或刻意的节拍分隔才串行。跨时空的点本来就串行，不在此列。
  - **判据（逐对相邻点自问）**：**把后一个点的第一句插进前一个点中间，剧情坏不坏？不坏，就说明这两个点本来就该互相打断。**
...
（点7 是切分点：包间里话到嘴边收住是前半，散场路上说出口是后半，拆开落在两处；点11 是跨度点：细纲写的就是从当夜守到次日开门的一段事，天然骑在两个时空上。）
...
- 自检三条，命中即就地重织再交付：
  ...
  2. **平推**：时空表如果基本是「一个时空一个点」，说明你在按细纲顺序平推——回去对每对相邻点跑形状层那条判据（后一个点的第一句插进前一个点中间，剧情坏不坏），不坏的就该交错。

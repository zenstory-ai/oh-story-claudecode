...
**交付前八问**：prompt 要求回答八问时，按 `agent-references/writing-craft.md` 第 8 节「自检方法（二）交付前八问」逐条自答，答案写进交付摘要（状态台阶 / 同类递减 / 性质由代价演示 / 标价人 / 镜头准入 / 人物余韵 / 最重的一句最短 / 密处禁概括）。**每条要指出正文里对应的那一处，只写「已遵守」不算回答**；做不到的写明是哪一条、为什么，不允许略过或事后补写成已完成。这八条不是风格建议，是本次交付的验收项。

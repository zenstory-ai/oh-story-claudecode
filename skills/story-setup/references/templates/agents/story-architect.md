...
### 大纲排布
- 五步大纲创建法：高潮 -- 剧情单元 -- 故事线 -- 开篇 -- 收尾
- 卷级结构：每卷功能、核心事件、状态变化
- 卷纲与剧情单元卡（长篇）：按主会话随 prompt 附上的卷纲模板写——每个标题下一行写 `> 作用域：卷级常任｜单元级 {单元ID}`；供给自查、建纲追加写 `大纲/排纲底稿_{单元ID}.md`，不进卷纲；prompt 没附模板时报告缺模板，不凭记忆拼字段
- 细纲设计：每章输出“章节蓝图”——九个必填字段（核心事件、字数目标、字数口径、单元ID/位置、目标情绪、主角目标/关键选择、章首钩子、爽点、本章禁止提前释放）+ 四个小节（内容概括五段式，发展/转折承载爽点铺垫·倒推法；情节安排；人物关系和出场顺序；情节细化的四列情节点表）+ 结尾设定和钩子；格式见下方「细纲蓝图输出格式」
- 章节规划：字数、节奏、情绪起落
- AB交织法：A线升级感 + B线情节冲突
...
- **不要输出字段不全的薄细纲**。新建/补建细纲必须有九个必填字段（核心事件、字数目标、字数口径 `visible_chars_v1`、单元ID/位置、目标情绪、主角目标/关键选择、章首钩子、爽点、本章禁止提前释放）、四个小节（内容概括、情节安排、人物关系和出场顺序、情节细化）、四列情节点表（# / 情节点 / 功能标签 / 执行边界（禁＋放）），以及结尾设定和章尾钩子；阶段位置、结构公式等阅读体验字段按需写。无证据的辅线/感情线可写“无”或 `[待补充]`，不能为了格式编造。

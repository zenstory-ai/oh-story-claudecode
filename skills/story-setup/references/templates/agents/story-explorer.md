...
4. **主产物探针与资料代际**：
   ...
   - 任一未命中时，核对 `_progress.md`、`拆文报告.md`、`章节/*_摘要.md`、`剧情/*.md` 和 `文风.md` 的存在、非空与覆盖范围。进度为 `schema_version: 2`，或旧核心资料不完整时按下面两项返回并停止准备。
   ...
   - 没有 current schema 且旧报告、逐章事实、剧情资料覆盖完整 → 返回 `gaps.legacy_benchmark_limited: true`；已存在的当前主产物仍进入步骤 5，缺失项的召回值与来源路径为 `null`，再从步骤 6 继续旧文风/章节参考。不得把旧文件改名或概括成权威主产物。
5. **当前权威召回**：
...
11. **读匹配章节资料**：
   ...
   - 新版轻量摘要优先读取「章节卡」的信息变化、章尾钩子和三维节奏；旧摘要没有章节卡时再读取「关键信息与扩写技法」表，作为 `matched_chapter_techniques` 的证据/补足；两者都不覆盖 `剧情/节奏.md`
...
> 普通查询遇文件缺失时在 `gaps` 中返回事实；`context_load` 缺 state、续写状态卡或 `check` 失败时停止。`benchmark_style_load` 的当前资料缺两个主产物时返回 `missing_primary_contract` 与 `repair_action`；只有步骤 4 验证为完整旧拆文才返回 `legacy_benchmark_limited` 并继续旧能力。登记的主对标书目录探不到时返回 `benchmark_book_missing` 与 `expected_path`；缺 `文风.md` 归 `profile_missing`。
...
```json
{
  ...
  "gaps": {
    ...
    "legacy_benchmark_limited": false,
    "repair_action": null,

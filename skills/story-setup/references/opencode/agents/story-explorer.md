...
加载对标书的情绪模块 + 节奏索引 + 文风 + 按本章情绪/基调匹配可参考章节 + 原文锚点片段；工作区有公共灵感库时附带跨书灵感聚合召回。
...
10. **多候选章节选择规则**（候选集多章时）：
   ...
   - L2 章容量接近：单情节点摘要比「三维节奏」篇幅展开度与本章计划展开度的接近度（目标字数相对本书常规章长映射高/中/低≈3/2/1）；多情节点摘要比情节点数 / 可读到的原文章节估算长度与本章目标字数（如提供）的接近度；本 agent 不用 Bash 统计，拿不到原文长度时跳过该信号，不得把摘要文件字数当原文字数
   - L3 章节号最小
11. **读匹配章节资料**：
   - 先 `Read {对标书路径}/章节/第K章_摘要.md`，提取本章基调序列、关键事件、爽点/情绪节点
   - 单情节点摘要直接读取「信息变化」「状态变化」「章尾钩子」「三维节奏」字段；缺这些字段的多情节点摘要再读取「关键信息与扩写技法」表，作为 `matched_chapter_techniques` 的证据/补足；两者都不覆盖 `剧情/节奏.md`
...
```json
{
  ...
  "results": {
    ...
    "selected_inspiration_aggregates": ["<Top 3 active CBA：ID、命中标签、机制链、可变参数、成立条件、风险、来源数量；无灵感库时为空数组>"],

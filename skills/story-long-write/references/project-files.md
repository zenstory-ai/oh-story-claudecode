...
├── 大纲/
│   ├── 大纲.md                # 全书卷级结构
│   ├── 卷纲_第一卷.md         # 每卷一个：卷契约+故事线+剧情单元卡+情绪弧线+人物弧线+伏笔+反转
│   ├── 排纲底稿_L1-01.md      # 每单元一份：供给自查+建纲追加
│   └── 细纲_第001章.md        # 每章一个：九个必填字段+四个小节+结尾设定和钩子
├── 正文/
│   ├── 第001章_章名.md
│   └── ...
├── 对标/                          ← 拆文产出的结构化资产
...
| 大纲/排纲底稿_{单元ID}.md | 剧情单元 | Phase 3；正文「细纲没给」追加进「建纲追加」 | 只在排纲/补纲时读 |
...
5. **伏笔/时间线文件缺失** → 视为当前状态损坏，停止写正文；先运行 `tracking_commit.py check`，再用事务修复。卷纲/大纲中的计划不能代替已发生的事实。

...
2. **标关键点**：看对标这几个一级结构在 1/4、中点、3/4 各放了什么关键情节，判为逆转 / 转折 / 激励。首选锚是 `剧情/节奏.md`「全书情绪节奏总览」的高潮分布与冲突升级路径；再用摘要里「类型=转折点」的情节点 + 基调突变（如温馨→压抑）与剧情单元的「结构分布」「情节点索引」作校验锚，不靠凭感觉。总览缺失回退纯摘要扫描；总览的爽点频率可作章节定位高低压分布参考。
...
建卡时做一次**剧情批召回**：读剧情单元卡「对标剧情参照」指向的剧情单元（≤3 个文件）的「结构分布」「情节点索引」，回查该书 `剧情/节奏.md` 同章节区间的爽点循环 / 触动点与被引情绪模块卡，提炼 5-8 拍共性节拍写入剧情单元卡「单元节拍/章功能分配」，单元情绪引擎字段对照主对标 `剧情/情绪模块.md` 的可复现模块校准。提炼节拍以「循环单元」中/大循环为骨架；有「三维节奏表」时取同区间篇幅展开度与展开/压缩手法写入章功能分配的展开取舍；信息揭示类单元另回查「关键双时间线」同区间披露路径（异常→线索→解释→确认）写入揭示/伏笔规划——三节任一缺失跳过，不记 gap 不阻断。若主产物存在但被引的单个情绪模块卡未命中，只跳过该卡；`剧情/情绪模块.md` 或 `剧情/节奏.md` 文件缺失仍按上方 fail-fast 规则先修复。换素材与修硬伤纪律同上方迁移步骤 3/4——保留情绪链和功能位，对象、场景、事件全换，对标弱处理就地修正。结论固化进剧情单元卡后，逐章细纲只消费剧情单元卡，不重读剧情单元。

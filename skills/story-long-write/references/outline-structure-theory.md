...
**「只消费剧情单元卡」管的是不重读对标，不冻结逐章建纲。** 排纲当场发现的缺口（某拍无对手、数值没验过、地点无底图、角色既定规矩挡场之类）按新增物三级处置（直接写／写了要报：先落档再引用／先问作者：挂起提案，见 workflow-setup.md「缺什么按新增物三级处置」），处置完记一行，批末汇进《供给单》的「**建纲追加**」小节（见 workflow-setup.md「按剧情批出细纲」步骤 5）；下一批供给自查以该小节为固定输入（按当场缺口类别加问批前盘点项）。

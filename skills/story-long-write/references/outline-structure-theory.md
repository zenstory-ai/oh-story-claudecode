...
## 一级结构 = 机制链在节奏曲线中的一次完整兑现

一级结构是一个完整单元剧：一个读者需求被触发、蓄力、转折并兑现，约 1.5-3 万字 / 数章。对标侧由 `structure_blocks.csv` 保存可变长度完整循环，`全局分析/爆款机制.md` 给出机制链，`全局分析/六维拆书.md` 的“三维节奏”章节给出块级峰谷和密度变化，五列 `chapter_index.csv` 只负责定位；本书侧仍落成卷纲里的剧情单元卡。
...
1. **选机制与曲线**：读主对标 `全局分析/爆款机制.md`、`全局分析/六维拆书.md` 的“三维节奏”章节与 `structure_blocks.csv`，按本卷读者需求和冲突类型选 1–3 个机制/结构块。跨书补充按 `cross-book-recall.md` 用任务标签只读 CBA，不遍历副书。
2. **标关键点**：用结构块的缺口、转折、兑现、剩余钩子与三轴强度定位 1/4、中点、3/4。需要确认时把块内章号通过五列 `chapter_index.csv` 映射到 `source_locator` 再读主对标原文；索引不含 `key_plot_nodes`、强度或钩子字段。
3. **换素材**：保留机制因果链、读者情绪链和功能位，把对象、场景、设定、事件与措辞全部换成自己的。可替换项与迁移条件取 `爆款机制.md`，禁止照搬具体桥段。
4. **修正硬伤**：换素材前读 `全局分析/证据与边界.md` 的反例、证据弱项、不可泛化条件和模仿风险。对标若在某关键点是弱处理，迁移时补足缺失的因果或提前必要铺垫，不把硬伤一起搬走。
5. **排章 + 落卷纲**：把换好（并修正）的关键情节落到本卷相同相对位置——这里的 1/4·中点·3/4 是**卷尺度**（1/4≈总章×0.25、中点×0.5、3/4×0.75）；每个本书剧情单元内部仍有自己的触发→蓄力→兑现→冷却。关键点之间用普通推进章过渡，峰值之后按需插呼吸 / 关系章；写进卷纲「对标结构坐标」与情绪弧线的章节定位列，并把 `{书名} / 机制ID / RH节奏段 / 证据章节范围` 写入剧情单元卡「对标剧情参照」。
...
建卡时做一次**剧情批召回**：从 `爆款机制.md` 提取触发链，从 `六维拆书.md` 的“三维节奏”章节和 `structure_blocks.csv` 提取峰前蓄力、峰值、冷却与完整循环；五列 `chapter_index.csv` 只把选中块的章号转成 locator。跨书机制只读标签命中的 CBA；只有主对标按 locator 读原文。结论写入剧情单元卡后，逐章细纲只消费剧情单元卡，不反复读取对标全书。

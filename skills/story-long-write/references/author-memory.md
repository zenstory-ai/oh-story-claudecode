...
- **写入不因预算失败**：`record` / `commit` 照常成功、给回执；工具按上表四类任务组合估算最坏查询情形（全局条目＋各 scope 维度最重的单一切片，轻重按写作时真正读到的字段算），装不进 2048 字节的组合在返回的 `warnings` 里点名将被略过的条目。收到提醒就转告作者并建议「整理作者记忆」。查询按 本书例外 → 重要度 → 最近更新 排序装填，被略过的恒是较旧且重要度较低的条目。
...
保留用户的否定词、限定词和适用范围，`quote` 写原话，`assertion` 只做不改变语义的紧凑归纳，**限一句话（≤120 字节，约 40 个字）**——一句原话里塞了几个偏好就拆成几条记录（例如「以后少用破折号，对话里也别用，除非表示打断」拆成两三条）；需要解释的背景写进 `reason`（不进 prompt 载荷），不另开字段。范围规则：
...
作者说「整理作者记忆」，或回执 `warnings`／查询 `omitted_ids` 提示超编时：读完整 `作者画像.md`，提出合并相似条目（`replace` 多合一）、退役过时条目（`forget`）、把超长断言拆短的提案，列成清单让作者逐条确认，确认后汇成一份 `commit` 事务一次提交。整理只由作者发起或确认，不自动执行。
...
- `record`：常用单事件入口，自动读取当前修订、首次自动初始化；`event_id` 相同且内容相同会幂等返回原回执，内容不同会失败。返回里的 `warnings` 是预算提醒，要原样转告作者。
- `query`：只读相关 active 条目；`--kind` 必传、可重复，不存在 state 时返回空结果且零写入。输出按 本书例外 → 重要度 → 最近更新 排序装填；装不下的条目跳过而不中断（一条长的不挡后面的短条），漏下的 ID 报进 `omitted_ids`（最多列 20 条，`omitted` 是真实总数）。**`omitted_ids` 非空＝记忆超编**——转告作者并建议「整理作者记忆」，它不是「没有更多了」，也不得改读完整画像规避预算。

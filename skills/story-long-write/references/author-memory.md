...
- **写入即限载**：`record` / `commit` 在落盘前按上表四类任务组合校验——每一组在最坏查询情形下（全局条目＋各 scope 维度最重的单一切片）都必须装进 2048 字节，超限**拒绝写入**并列出该组合最重的条目。此时先盘点：精简判据句、合并同义条、退役过时条，再写。这道闸保证「查询恒完整」——记忆写得进去，就一定载得进 prompt，不存在写入成功却被静默截断挤掉的条目。
...
- `query`：只读相关 active 条目；`--kind` 可重复，不存在 state 时返回空结果且零写入。写入即限载保证输出恒完整；装不下的条目跳过而不中断（一条长的不挡后面的短条），漏下的 ID 逐个报进 `omitted_ids`。**`omitted_ids` 非空＝老库超编还没盘点**，先精简/合并/退役再重查——它不是「没有更多了」，也不得改读完整画像规避预算。

...
- **模块召回、题材卡与文风召回**：
  - **成熟项目短路径（召回降档）**：`custom_style=true`、题材卡有内容，且细纲目标情绪与单元卡情绪/节拍均有效时，**跳过 (a)(b)(e)(f)**；情绪和节奏取细纲/单元卡，(a0)(c)(d) 照常执行，(g) 输出缩减集。组装脚本判定并写入核对报告；条件不足走全量召回。降档不检查对标情绪/节奏主产物，也不触发其 fail-fast；代价是没有匹配章扩写技法与原文锚点。目标情绪敷衍时不得降档。
  - ① 本章目标情绪词？② 借鉴哪个参考文件的哪个技法？③ 用在哪些段落？答不出 → 先回读参考再动笔
  - (a0) **公共灵感标签召回**（可选轴，有 `灵感库/` 才执行）：按 `references/cross-book-recall.md`「公共灵感标签召回」从本章题材、读者需求、目标情绪、关系动作、剧情功能、节奏位置、`适用阶段=正文` 和风险生成标签查询；只读 Top 3 张 active CBA 全卡，输出 `selected_inspiration_aggregates`。不沿来源 ID 回查 EM/NM；灵感库不存在或零命中只记 `gaps.inspiration_library_missing` / `inspiration_tag_no_match`，不阻塞
  ...
  - (e) **匹配章节挑选**：从 `{对标书路径}/章节/*_摘要.md` grep `基调：(紧张|轻松|悲伤|热血|爽|甜|温馨|恐怖|压抑|其他)`（全角冒号），按本章目标情绪挑章 K——多章同基调时：先比爽点类型，再比章容量——单情节点摘要按「三维节奏」篇幅档贴合计划展开度，多情节点摘要按情节点数量/估算字数接近目标字数——最后取章节号最小者；必读 `{对标书路径}/章节/第K章_摘要.md`——单情节点摘要读「信息变化/状态变化/章尾钩子/三维节奏」字段作参考，多情节点摘要读「关键信息与扩写技法」表；若同章存在 `第K章_深度拆解.md` 则加读，否则回退黄金三章深度拆解/文风文件里的可借鉴技巧，不因非黄金三章缺少深度拆解而失败
  - (f) **结构化模块召回**：从对标的结构化子目录（角色/设定）按本章情节检索相关模块（剧情单元已由材料清单选定，不重复检索）；若与 `剧情/情绪模块.md` / `剧情/节奏.md` 冲突，权威文件优先，记录 `conflict`
  - (g) 输出"selected_inspiration_aggregates + 主对标召回摘要 + 副对标召回摘要 + selected_emotion_module + rhythm_reference + genre_prose_card + 文风召回指令 + 原文锚点片段引用"，作为 narrative-writer 的输入。`selected_inspiration_aggregates` 只影响设计选择，不覆盖细纲事实、项目设定或自定义文风。**多对标书时**参 `references/cross-book-recall.md`：主对标提供文风、原文锚点及可用的情绪/节奏值；副对标/参考对标按阶段预算提供结构化摘要，不限制登记书目，不读取副书 `文风.md` / 原文，超过预算时裁条目不裁书目记录。
  - **快捷路径**：项目已部署 story-explorer agent 时，可一次性召回文风/模块材料。
    ...
    - 需要返回：`style_profile_path`、`style_profile_summary`、`selected_emotion_module`、`rhythm_reference`、`selected_inspiration_aggregates`（无灵感库时为空并记 gap）、来源路径、匹配章节、锚点片段、`gaps`。

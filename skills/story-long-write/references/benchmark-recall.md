...
- **模块召回、题材卡与文风召回**：
  - **旧成果路径**：`project-files.md` 确认 `legacy_benchmark_limited` 后跳过 (a)(b)，两个值为 `null`；(c)-(f) 继续读旧资料，(g) 标记限制。不得生成空壳；需要新召回时再增强。
  ...
  - (g) 输出"主对标召回摘要 + 副对标召回摘要 + selected_emotion_module + rhythm_reference + genre_prose_card + 文风召回指令 + 原文锚点片段引用"，作为 narrative-writer 的输入；旧成果路径另标 `legacy_benchmark_limited: true`。**多对标书时**参 `references/cross-book-recall.md`：主对标提供文风、原文锚点及可用的情绪/节奏值；副对标/参考对标按阶段预算提供结构化摘要，不限制登记书目，不读取副书 `文风.md` / 原文，超过预算时裁条目不裁书目记录。
  - **快捷路径**：项目已部署 story-explorer agent 时，可一次性召回文风/模块材料。
    ...
    - `gaps.missing_primary_contract` 为 true 时按 `repair_action` 修复并停止生成；`gaps.legacy_benchmark_limited` 为 true 时按旧成果路径继续。

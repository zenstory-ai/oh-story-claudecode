...
每次设定、卷纲、细纲或正文任务先生成需求标签。轴以 `灵感库/灵感索引.csv` 的标签轴契约为准；**值必须取自 `灵感库/标签词表.md` 的受控值**——按下表定位本书语境后，映射到词表中最贴近的值再组查询，不自造、不改写同义词（query 靠精确匹配，自造词等于查不到）；词表文件缺失时以 query 返回的 `axis_inventory` 现存值为准：

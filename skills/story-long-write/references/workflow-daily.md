...
- 项目已有 `正文/` 和 `追踪/` 目录
- 用户明确说"日更""续写""继续写/接着写"，或明确指定"写第 N-M 章"；上一轮在规划时，"继续写"先按 SKILL.md「规划续接」用一句话确认
...
2. **逐章执行**：每章按 [workflow-chapter.md](workflow-chapter.md) 的「单章写作流程」步骤 1-13 完整走一遍（写前准备、正文执行、字数验证、钩子/爽点检查、元信息与禁用词扫描、追踪事务）。批量模式下再叠加下列日更专属动作：
   ...
   - 每章写完后**立即提交一次追踪事务**：
     1. 事务 JSON 写到本章工作目录 `.story/work/第NNN章/tracking.json`（见 workflow-chapter「工作目录」），不放书根或 `大纲/`。从刚落盘的正文、细纲和上一版续写状态卡提取 `result / character_changes / foreshadow_changes / timeline_events / constraints / next_chapter_commitments`。只记录会影响未来章节的变化；过程日志、质检计数、参照章和去 AI 味统计全部排除。
     ...
     4. 运行 `storyctl.py chapter check`，取 `state_revision` 写入无 `wordcount` 的事务。带内 + quality pass → `chapter commit`；`under` 不补，`over` 按 workflow-chapter 步骤 8 净删一次并复检；仍带外走该步骤三动作。提交会重读、重数、重跑 quality；成功后脚本自动删除本章工作目录，tracking 提交后才继续。
     5. 失败时 `_tracking-state.json` 尚未推进；工作目录原样保留，修正写入环境后重跑同一 `commit`。不得另写下一章、不得手工补派生视图、不得忽略返回码。
...
3. **不中断但不并发**：tracking 已提交即进下一章（除非用户要求确认）；`under` 或一次压缩后仍不在范围内，按 workflow-chapter「向作者汇报」的字数问法请作者选，不静默推进。下一章先读上一章正文与追踪更新。
...
4. **一致性检查（整批一次）**：本批各章没有按需调用过 consistency-checker 的，在这里 spawn 一次，检查范围写本批全部章节，prompt 同 workflow-chapter「Agent 调用：consistency-checker」（申报栏写各章申报汇总）；各章都已单独查过就跳过。查出的冲突照该节三档处置。

批末只对本步改过的章重跑 `chapter check --fix-punctuation`；没改过的章提交时已通过检测，不再整批复扫。
...
然后按 SKILL.md「面向作者的汇报」口头汇报，不另存为文件。上面的分流、漂移与盘点只是内部核对，翻成故事话再说：
...
3. 读取最新一章正文（情节衔接）
4. 按 workflow-setup.md「Phase 3：大纲搭建」的新版细纲模板补建本章细纲，补齐阶段位置、本章结构公式、本章禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定；无法从卷纲/正文/设定确认的字段写 `[待补充]`，不杜撰
5. 补建完成后继续 Step 2 写作

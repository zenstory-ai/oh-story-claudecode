...
> 先按 `SKILL.md`「写作流程」核定当前任务；裸调用不进日更，当前仅规划或只改细纲也不因已有正文而进入本 workflow。
...
**批量 continuation 规则**：未改变任务时，"继续/续写/日更"仍执行完整批量流程，不跳过状态筛选或对标模块/节奏/题材卡/文风召回，不逐章重复确认（用户要求或阻塞除外）。用户转为只规划/改细纲时退出批量，按 SKILL.md 的范围与续接规则执行。到达本轮 K（最多 3 章）后必须进入 Step 3/4 收尾并停止，不因还有细纲续写下一批。

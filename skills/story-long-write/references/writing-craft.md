...
- 逐点的目标字数是**排纲时的分量刻度**，不是正文验收单位。**不要为了让某个点单独凑够字数，把它写成一块连续的独立段落**——那正是平推的成因。
...
- **同一时空内（时间连续、地点相同）的相邻情节点默认合并、穿插、重排**，织进同一段连续的戏；把它们排成一前一后的串行**才是需要理由的那一侧**——只有因果依赖（B 必须在 A 之后才成立）或刻意的节拍分隔才串行。跨时空的点本来就串行，不在此列。密点展开成慢镜头，疏点带过（见「疏密分配」）。
  - **判据（逐对相邻点自问）**：把后一个点的第一句插进前一个点中间，剧情坏不坏？**不坏，就说明这两个点本来就该互相打断。**
  - 例：一场饭局里「主角当众被驳了一次」「邻座起哄」「同伴趁空要回垫付的钱」「同伴顺口说出一个关键消息」四个点，全落在同一顿饭的那段时间里——该让它们互相打断（要钱插在两次举筷之间、消息垫在被驳之后），而不是吃完了再让人开口。

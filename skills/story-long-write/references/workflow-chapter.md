...
2. **读取上下文**（按需选择；缺失时遵循各项及 `project-files.md` 的「缺失文件处理」，仅明确标为可选的非主产物跳过。可选快捷路径：对应 agent 已部署、**且降档不成立**时 spawn story-explorer 一次获取上下文。Prompt：`项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章\n追踪状态：last_committed_chapter={check 的值}，state_revision={check 的值}`）：
   ...
   - (6) `参考资料/{topic}.md`（如存在）— 历史研究资料（由 story-researcher 产出）
   ...
   - (10) 对标书路径下 `剧情/情绪模块.md`（按对标书路径查找）— 仅全量召回；读取与选取在 3(a) 一次完成，本项只列路径与阻断链；读者需求 / 情绪引擎、爽文套路框架、可复现模块；缺失按 `project-files.md` 的「缺失文件处理」设置 `missing_primary_contract` 并停止准备
   - (11) 对标书路径下 `剧情/节奏.md`（按对标书路径查找）— 仅全量召回；读取与选取在 3(b) 一次完成，本项只列路径与阻断链；关键信息推进、情绪触动点、爆发节奏；缺失按 `project-files.md` 的「缺失文件处理」设置 `missing_primary_contract` 并停止准备
   - (12) `设定/题材正文提示卡.md`（如存在）— 本书正文层题材卡。**加工在 3(c)，本项只列路径与兜底链**，读一次就直接产出 `genre_prose_card`，不在这里先读一遍再去 3(c) 召回一遍；缺失时从 `设定/题材定位.md` + `references/genre-prose-cards.md` 索引 + `references/genre-prose-cards/` 单题材卡目录（按题材分类优先）+ `references/style-genre-modules.md`（兜底）即时生成 `genre_prose_card`，不阻塞写作
...
7. **正文执行**：
   ...
   - **主会话填八槽**（搜 `［主会话填］`）：执行安排 ／ 本章意图 ／ 参考技法 ／ 本节速记 ／ 涉及角色 ／ `genre_prose_card` ／ 必读设定 ／ `author_preferences`。填法见槽内提示。作者偏好仅作低优先级倾向，不逐条展示或最大化命中，不牺牲连贯、节奏和字数。降档不成立时情绪与节奏槽也是空槽；工作区有 `灵感库/` 时另有「跨书灵感」空槽（按 (a0) 填 `selected_inspiration_aggregates`，零命中写「无」），无库时脚本已封槽。填完**空槽以外一字不改，整份照抄进 Agent prompt**；`--out` 留档文件供事后比对。

...
7. **正文执行**：
   ...
   - 如可用，spawn `Agent(subagent_type: "narrative-writer", prompt: ...)`，prompt 只传本章必需材料：
     ...
     - **交付前八问（写法层）**：这行是**指令**不是材料栏目。prompt 里写明「按 `writing-craft.md` 第 8 节「自检方法（二）交付前八问」逐条自答，**答案写进交付摘要**」。八问覆盖的是最容易被当成风格建议读过去、因而整条落空的写法条目：状态台阶 / 同类递减 / 性质由代价演示 / 标价人 / 镜头准入 / 人物余韵 / 最重的一句最短 / 密处禁概括。**只写「已遵守」不算回答——每条要指出正文里对应的那一处；做不到的写明是哪一条、为什么，不允许略过。** 交付摘要缺八问 = 这行没传，重 spawn。

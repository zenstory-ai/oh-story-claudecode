...
7. **正文执行**：
   ...
   - **主会话填七槽**（搜 `［主会话填］`）：本章意图 ／ 参考技法 ／ 本节速记 ／ 涉及角色 ／ `genre_prose_card` ／ 必读设定 ／ `author_preferences`。填法见槽内提示。作者偏好仅作低优先级倾向，不逐条展示或最大化命中，不牺牲连贯、节奏和字数。降档不成立时情绪与节奏槽也是空槽。填完**空槽以外一字不改，整份照抄进 Agent prompt**；`--out` 留档文件供事后比对。

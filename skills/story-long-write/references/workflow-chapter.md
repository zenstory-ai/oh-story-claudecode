...
6. **写作与唯一 checkpoint**：将批准情节点按叙事顺序分成连续两组——**切分点尽量选在时空边界**（细纲情节点的时间跳转或地点切换处），不把同一时空内本应交错的相邻点拆进两组，无明显边界再按叙事中点分——同一 session 先写前组临时 segment，再且只运行一次 `{PYTHON} {skill 根}/scripts/storyctl.py wordcount checkpoint --file {前组临时文件} --target {字数目标} --chapter {N}`。把 `actual`、`remaining_user_range` 和后组原始情节点交回 writer，并明确：
...
7. **正文执行**：
   ...
   - 如可用，spawn `Agent(subagent_type: "narrative-writer", prompt: ...)`，prompt 只传本章必需材料：
     ...
     - 细纲优先边界（内容层）：只展开本章细纲，不自造新剧情；每条情节点都要落地演足，不许漏、不许把两条并成一句话交代过去（这条防漏、不防交织：拆开、交错、互相打断照样算各自演足）。不得仅为追字数自动补纲、扩写或重写；实际长度统一留到步骤 8 测量。
     - 正文形状（形状层）：落地位置、顺序、拆成几处由子代理编排。**同一时空内的情节点默认互相打断、交错落地，不局限于相邻两点——合理范围内块内任意点可穿插；串行才是需要理由的那一侧**（因果依赖／刻意的节拍分隔）。判据写进 prompt：**把乙点的一句插进甲点中间，剧情坏不坏？不坏就可以互相打断；因果被打乱、信息提前泄露、情绪节拍被冲散就不插**。不把细纲措辞原样搬进叙述。
     - 要求随交付返回**时空表**（子代理自己划的时空块 → 各含哪些点，格式与横跨标注见 narrative-writer「写完后编排自检」），供步骤 8 核漏项与形状。
...
8. **先核时空表，再非对称收口**：
   - **核时空表**（交付确认）：细纲每个情节点是否**至少出现一次**——有漏即退回补写（补的是批准内容，不是加戏）；横跨多个时空的点须标注，每一处都要有实在内容。若时空表基本是「一个时空一个点」，说明子代理按细纲顺序平推了，退回按形状层判据重织。顺带核组装接缝：前后组切分点应与表中某两块的边界吻合，接缝两侧不得出现动作或称谓的重复交代。这些退回属于覆盖/形状修复，不以字数为由发起。
...
字数权威为细纲 `字数目标` + `visible_chars_v1`：内带 ±12%，用户带 ±15%；无节奏下限，缺目标即 `invalid`。带内提交，`under` 不补，`over` 最多净删一次；仍带外由用户接受、改目标/细纲或放弃。下一章只看 tracking 提交。

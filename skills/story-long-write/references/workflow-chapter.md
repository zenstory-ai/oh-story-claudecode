...
2. **读取上下文**（按需选择；缺失时遵循各项及SKILL.md Phase 4 的「缺失文件处理」，仅明确标为可选的非主产物跳过。可选快捷路径：对应 agent 已部署、**且降档不成立**时 spawn story-explorer 一次获取上下文。Prompt：`项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章\n追踪状态：last_committed_chapter={check 的值}，state_revision={check 的值}`）：
   ...
   - (11) 对标书路径下 `剧情/情绪模块.md`（按对标书路径查找）— 仅全量召回时读取；读者需求 / 情绪引擎、爽文套路框架、可复现模块；缺失按SKILL.md Phase 4 的「缺失文件处理」设置 `missing_primary_contract` 并停止准备
   - (12) 对标书路径下 `剧情/节奏.md`（按对标书路径查找）— 仅全量召回时读取；关键信息推进、情绪触动点、爆发节奏；缺失按SKILL.md Phase 4 的「缺失文件处理」设置 `missing_primary_contract` 并停止准备
...
3. **写前准备**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - **第 0 步：先跑组装脚本**（可用时）。`{PYTHON} {skill 根}/scripts/build_writer_prompt.py --project {项目根} --chapter {N} --out {留档文件}`，脚本统一输出 UTF-8。stdout 的 `====` 分隔线以上是 prompt 正文、以下是核对报告（注入了哪些块、跳过哪些、为什么，以报告为准）。脚本已做完的部分下面不再重做，主会话只填空槽（搜 `［主会话填］`）。退出码 2 ＝ 数据问题，先修数据再重跑，不得手拼绕过；脚本跑不起来才按步骤 7 回落。
   ...
   - **模块召回、题材卡与文风召回**：
     ...
     - (d) **文风召回**：先直接读 `设定/文风.md`（不经 explorer）：含实质内容（去空白 ≥200 字，或含 句长 / 标点 / 对话 / 锚点 / 笔调 小节且小节内有可执行约束：比例 / 例句 / 禁止或偏好描述）则置 `custom_style=true`、进入「自定义文风模式」，它作权威风格基（句长 / 软标点 / 潜台词 / 情绪交替），对标 / 拆文 `文风.md` 降为参考（锚点 + 句长兜底）；空 / 仅空白 / 仅标题 / 占位 stub（待办 / 待补充 / ___）视为不存在。**自定义文风模式下若存在 `设定/_文风摘要.md`**（书级文风摘要卡：硬规则＋句长带＋对话落法的压缩版，由作者侧从文风.md 生成维护），写作 spawn 的 `文风路径` 传摘要卡、全文只在冲突时定点查；组装脚本已按此处理。否则按「对标书路径查找」规则读 `{对标书路径}/文风.md`（路径优先 `{项目}/对标/{书名}/`，回退 `拆文库/{书名}/`）；多本对标书时从 `设定/题材定位.md` 读 `主对标书` 字段。**未进入自定义文风模式且**文风文件不存在 → **fail-fast 报错**：「对标书 X 缺少 文风.md。请用 `/story-long-analyze` 跑 Stage 6 生成文风，再 `/story-import` 同步。」不 inline 生成（自定义文风模式则不 fail-fast；情绪 / 节奏轴 `missing_primary_contract` 仍独立阻塞）
...
6. **写作准备**：批准情节点按叙事顺序**一次性全部交给 writer**，同一 session 内一次写完整章后落盘。
   - 字数目标是**整章的分量刻度**，全章疏密由 writer 自行分配——哪些点铺开、哪些点一句带过由它决定；主会话只下发目标本身，不拆逐情节点配额。
   - 情节点全部落地即停。不得为字数新增独立事件、人物决定、关系变化、揭示或支线；仍欠长也不加剧情。
   - 不做字数 retry、完整重写或落盘后扩写；实际长度在步骤 8 由 `chapter check` 一次测量，欠长走该步骤的用户决策，超长走一次净删压缩。
   - **不在章中切分**：整章一次成文（交错与疏密判断以整章为单位，见步骤 7 形状层）。
...
7. **正文执行**：
   ...
   - **组装走脚本，不逐条手拼**：骨架在步骤 3 第 0 步已生成。脚本注入八项：固定首行、定位、**标题行字面量**（层级与章号补零位数从既有章学，不按通用规则推导）、细纲路径、文风四行（路径／优先裁决／停读清单／判读的通用参考）、上一章结尾切片、降档成立时的情绪与节奏召回、固定块指针。**细纲全文、卷纲单元卡、续写状态卡都不进 prompt**；agent 定义里已有的只留指针（见下方规格）。可选块缺失留标题并写明原因。
   - **主会话填七槽**（搜 `［主会话填］`）：本章意图 ／ 参考技法 ／ 本节速记 ／ 涉及角色 ／ `genre_prose_card` ／ 必读设定 ／ `author_preferences`——对应步骤 3「写前准备」的四项输出加三项。**每槽怎么填写在槽内提示语里，此处不复述。**降档不成立时情绪与节奏槽也是空槽。填完**空槽以外一字不改，整份照抄进 Agent prompt**；`--out` 留档文件供事后比对。
   ...
   - agent 在同一 session 内**一次写完整章**，直接写入 `正文/第XXX章_章名.md`；主会话核对它只消费了批准情节点。如 agent 未部署，由主线程执行同一流程。
8. **先核时空表，再非对称收口**：
...
质量检查阶段，consistency-checker 已部署时，仅按当前运行时的 canonical agent 目录检查并 spawn，获取 S1-S4 报告。Prompt：`项目目录：{dir}\n检查范围：{本次写作的章节}\n检查类型：事实冲突+伏笔断线+角色属性不一致\n本章新增申报：{writer 交回的申报表原样粘贴，无则写 0}——对表内每一项额外跑一遍你现有的实体/设定/时间线/规则边界/跨章因果/代价一致性扫描，冲突按 S1-S4 报出`。不可用则主线程参照 long-chapter-quality.md 直接检查。

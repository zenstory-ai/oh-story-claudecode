...
> **细纲钩子来源**：优先沿本批剧情单元卡「对标剧情参照」剧情单元的节拍与兑现方式设计钩子链；多对标书时参 `references/cross-book-recall.md`，副对标同基调 `章节/*_摘要.md` 仍可作单章钩子灵感（可选）；有 `灵感库/` 时可按「适用阶段=细纲」召回 active CBA 辅助钩子与兑现设计，零命中不阻塞

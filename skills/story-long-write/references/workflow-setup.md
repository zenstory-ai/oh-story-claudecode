...
> **对标节奏回流（有主对标书时，卷纲定稿前做一次）**：读主对标 `对标/{书}/剧情/节奏.md`；资料缺失时设置 `missing_primary_contract` 并停止，不得以摘要、故事线或报告代替。文件存在时，按 references/outline-structure-theory.md「对标节奏迁移」将 1/4·中点·3/4 换素材排入卷纲「对标结构坐标」，选中单元写入「对标剧情参照」（也可填「自创：{理由}」），并过安全七检。未登记主对标但有同题材候选时先回 Phase 1；确无对标则按八节点自排。

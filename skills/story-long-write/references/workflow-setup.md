...
- **追踪事务初始状态**：仅首次细纲完成、尚无正文且无 state 时初始化；只讨论/只做大纲不执行，已有追踪不重建。先完整读取 [tracking-initialization.md](tracking-initialization.md)，构造第 0 章 JSON，执行 `scripts/tracking_commit.py init` 后 `check`，通过后删除事务 JSON；核心角色动态快照在首次正文事务中创建。

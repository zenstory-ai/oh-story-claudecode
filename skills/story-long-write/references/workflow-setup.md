...
5. **建纲当场缺口**（逐章授权＋留痕，与步骤 4 并行发生）：步骤 4 的「只消费剧情单元卡」限制的是不重读对标（不冻结本章处置）。当场按「缺什么按创作自主权分级处置」办，不许硬写、不许 `[待补充]` 了事、也不许停下整批：
   - **A 级**直接写进本章细纲，不落档、不上报、不占《供给单》。
   - **B 级**先增量补进对应设定文件，**再**在细纲里引用（顺序固定：先落档后引用）；同时在手边记一行。
   - **C 级**不自裁——**包括推翻既有设定文件里已经写死的东西**（见分级第 3 条）。挂起该处，本章细纲先按既有口径写或留出接口，提案批末一并交作者。
   - **留痕**：本批建完，把这些当场处置汇成《供给单》的「**建纲追加**」小节（与批前六行分列，不混写），一条一行：`第 N 章排到哪一步发现缺什么 ／ 判为 A・B・C ／ 落在哪个文件或提案编号`；一条也没有就写「无追加」。
   - 下一批做步骤 3 时先读本小节（按当场缺口的类别加问批前盘点项）。
6. **批末结构复核**（每批建完必做）：按现有大纲流程确认章级目标与 `visible_chars_v1` 口径、核心字段、禁止提前释放和可执行情节点齐全。不得用“每个情节点固定等于多少字”预测容量或自动补事件；每章细纲落盘后必须跑 `node scripts/check-outline-contract.js --json --project {书目录} --chapter {N}`：exit 1 只按 `repair_scope` 补点名字段再复验，最多两轮；advisory 项逐条人工判断，不强制归零。批末另跑一次 `node scripts/check-outline-contract.js --json --supply {卷纲路径} {单元ID}` 验证本批《供给单》已落卷纲（含「无缺口」情形）；**「建纲追加」小节回填后再跑一次**（脚本只验小节存在，不验内容，这一项靠人）。**旧章细纲缺字段是常态，不要照抄同目录既有文件的字段集**，以本节模板为准。语义仍不设 checker。
...
- **大纲/卷纲_第X卷.md**：每卷的剧情单元+情绪弧线（含章节定位）+人物弧线+伏笔+反转+对标结构坐标+**刻度台阶/打脸债/压力线轮转**三表（按需填写，不适用时省略或写「无」；参考 outline-methods.md「大纲三层结构法」 + outline-structure-theory.md「章节定位与张弛 / 对标节奏迁移」 + emotional-arc-design.md「六种弧线速查」 + long-reversal.md「反转类型」；三表模板见 artifact-protocols.md）

...
先确定当前请求的操作对象、交付范围和停点，再看项目状态；项目为空不等于授权完整开书。以下是范围与转入正文的权威规则。
...
**规划续接**：在当前任务上下文保留交付范围；"继续/继续写/按这个来/确认方案"仍处理该范围，已完成就报告并停止，不自动扩章、转细纲或转正文。新的明确请求可调整规划层级/范围；转正文须明确要求（如"开始写第1章/日更2章/修改第3章正文"），无须再问是否继续。不把规划模式写入追踪或作者记忆；规划委派传相同范围和停点，不调用 narrative-writer。
...
**正文匹配**：确定已要正文后，按 大修 → 写指定章 → 日更续写 匹配；"开书并写首章"保留组合执行。日更要求已有正文+追踪，不满足时提示先补前置或写第1章，不默认生成一批正文。

...
用户未要求一次跑完时，Stage 1 后按 author-facing.md「开头三章拆完、停下来问」询问是否继续，并一并请作者选 Stage 2 派发方式（串行 / 有限并行 / 不限批次顺序，见 pipeline-ops「执行与提交一个批次」）；作者没选、要求一次跑完、多本书一起拆或由导入自动续跑时都按不限批次顺序（同时最多 12 个子代理，不超过环境上限），后三种情况不停下询问；续跑不重复 Stage 0/1。Stage 3–5 不重读原文。Stage 6 可按索引定点读取 4–6 段原文锚点，但不重扫全书。

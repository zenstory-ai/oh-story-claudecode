...
> 每片段 300-500 字，**用于 narrative-writer 写作时的范例片段**。只能复用旧文风、黄金三章或批次中已经保存并带定位的连续原句；Stage 6 不重新切原文。模仿手法、不抄字句。

...
2. **分派**：每批一个 chapter-extractor，只照抄计划里该批的 `source_locator`、字数、情节点下限、输出文件路径和交接缓存路径；子代理自己读原文与交接批的跨批状态。串行、有限并行或不限顺序由作者在 Stage 1 后选定（见 pipeline-ops「执行与提交一个批次」），并行时边界处的剧情点接续交给下方跨块合并。每个原文范围只有一个 owner。
3. **双层落盘**：同次产出逐章紧凑事实与批次观察，由子代理直接写进批次输入文件、只回回执；主会话不转贴原文、不读输入文件，直接提交。禁止另起一次模型调用重复分析同一批原文；兼容文件由提交脚本机械生成。
4. **主会话合并**：主会话用 `digest` 消费全书跨章观察和按窗口抽取的逐章字段做全局合成。全书仍超上下文时按章节窗口分组合并，中间层只保留剧情状态、关键候选和证据引用，不扩写内容。

...
> **语料读取**：读取 `_analysis_cache/批次-*.md` 中已经降维的跨章观察。新流程只在关键证据冲突时定点读取对应逐章兼容文件，最多增加一次定点核验，不重新扫描原文。旧项目没有批次缓存时，直接复用现有剧情、节奏、情绪、关系和逐章资料；证据不足或冲突时标未知或待核。Stage 3–5 不回读原文；Stage 6 可以按索引定点读取 4–6 段文风锚点，但不得重新扫描整本原文。
...
Stage 3-5 的语料是 `_analysis_cache/批次-*.md` 中的完整批次结果；只有关键冲突才定点读取对应 `章节/*_摘要.md`。旧项目没有缓存时直接读取已有剧情、节奏、情绪、关系资料；不要为了统一格式先把所有旧摘要重新压缩一遍。

...
- 使用 `scripts/build_chapter_index.py` 识别章界、标题、卷、本地章号、行号和正文字符数
- 在 `chapter_index.csv` 记录源 hash 与解析器版本；相同来源复用，不重复解析
- 机械索引不包含剧情、人物、情绪、钩子或模型推断

### 阶段 2：连续章节块与已有成果二次提取

先运行 `plan_analysis_run.py`，只执行计划中的批次。Stage 2 不再存在逐章独立调用。
...
- 普通短章通常 5-10 章；
- 长章或信息密集时 1-5 章；
- 每块最多 10 章，默认同时受 25,000 字符上限约束；
- 遇到卷界、来源缺口或空章必须断开；
- 每章在全书运行计划中只属于一个原文块。

同一次理解产生两层结果：

1. 每章紧凑事实：概要、因果、信息变化、状态变化、三维分数、钩子和证据；
2. 批次观察：剧情点、关键事件与披露候选、关系、三维异常点和跨批状态。

逐章兼容文件由脚本从紧凑事实机械生成；批次缓存保留全部逐章紧凑事实和跨章观察，使全局聚合不必逐个打开章节文件。不得第二次读取该块来生成逐章文件，也不得把 50-60 章交给一个 agent 后让它自行拆成多个“批次”。
...
逐章投影只保留现有导入、写作与查询实际使用的内容：概要、关键事件、情节点、基调、章尾钩子和证据。取消逐章重复的长篇写法公式、角色复述、扩写技法长表和大段引用。写法机制、信息差、关系与节奏集中在剧情点及 Stage 3 权威产物中。
...
> **语料读取**：读取 `_analysis_cache/批次-*.md` 和 `_analysis_cache/复用提取-*.md` 中已经降维的跨章观察。新流程只在关键证据冲突时定点读取对应逐章兼容文件，最多增加一次定点核验，不重新扫描原文。旧项目没有批次缓存时，直接复用现有剧情、节奏、情绪、关系和逐章资料；证据不足、冲突或无法判断具体写法时标未知或待核，不回读已覆盖章节原文。Stage 3–6 不得重新扫描整本原文。
...
2. **`剧情/情绪模块.md`（模块权威）**
   ...
   - 输出：读者需求 / 情绪引擎、故事框架与套路运行图、最强三个完整模块卡、其他机制索引、重组与复现指南。
   - 生成法：先抽象“读者为什么期待兑现”，再把具体剧情抽成情绪链和功能位；前三强模块卡必须包含必要铺垫、成立条件、可替换项、失效情形、反例和不可照搬项，其他机制只留用途、范围和证据引用。
...
|------|------|--------|
| 普通章节、上下文充足 | 连续读取并保留跨章状态 | 5-10 章/批 |
| 长章、信息密集或上下文较小 | 缩小批次，不截断明显短因果链 | 1-5 章/批 |
...
每章只保存一组紧凑事实。每批全部文件落盘并校验后更新收据、批次账本和 `_progress.md`。

### 处理批次（一次读多少）

Stage 3-5 的语料是 `_analysis_cache/批次-*.md` 和 `_analysis_cache/复用提取-*.md` 中的降维跨章观察；只有关键冲突才定点读取对应 `章节/*_摘要.md`。旧项目没有缓存时直接读取已有剧情、节奏、情绪、关系资料；不要为了统一格式先把所有旧摘要重新压缩一遍。

**A. 连续批次模式**

1. **切批**：按 `chapter_index.csv` 连续取 1-10 章，根据原文字数和上下文缩放；完整任务不跳章。
2. **分派**：每批一个 chapter-extractor，输入该批原文、边界和上批状态；每个原文范围只有一个 owner。
3. **双层回传**：同次返回逐章紧凑事实与批次观察，禁止另起一次模型调用重复分析同一批原文；兼容文件由提交脚本机械生成。
4. **主线程合并**：主线程一次消费所有紧凑批次结果做全局合成。只有批次总量仍超上下文时才先做分组合并，且中间层只保留剧情状态、关键候选和证据引用，不扩写内容。
5. **进度**：开始批次时登记 `running`；失败写 `failed`，必要时只拆分该块；成功后以收据 hash 登记 `success`。中断可按首个未验证批次续跑。

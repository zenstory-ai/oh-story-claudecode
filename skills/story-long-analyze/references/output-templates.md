...
同一对角色的最新状态和历史演变都保留；“甲保护乙”与“乙依赖甲”分别记录。以此文件为唯一关系事实源，默认生成 mermaid 版 `人物关系图/核心人物关系.md` 与 `人物关系图/关键关系演变.md`（graph 代码块），`拆文报告.md` 嵌入 mermaid 块；仅用户明确要求图片时才渲染同名 PNG，渲染失败降级为 mermaid 并记录原因，不编造图片。
...
- **人物关系及演变**：嵌入两张 mermaid 关系图（用户要求图片且渲染成功时用 PNG），摘要关系方向、触发、双方得失和阶段变化

...
同一对角色的最新状态和历史演变都保留；“甲保护乙”与“乙依赖甲”分别记录。关系图只由此文件生成，不手画、不另建关系事实：运行 `scripts/render_relation_chart.py --root "{拆文目录}" --png`。它总是写出 `人物关系图/人物关系图.md`（Mermaid 图 + 文字清单，任何 Markdown 查看器都显示中文，作为主产物）；只有装了 matplotlib 且找到真正含中文字形的字体时，才另画 `核心人物关系.png` 与 `关键关系演变.png`。找不到中文字体就不出图，绝不改用拼音或首字母，并把脚本返回的说明转告作者。

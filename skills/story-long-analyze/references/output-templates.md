...
同一次黄金三章阅读另写 `_style-sample.txt`，不要在 Stage 6 回头切原文：
...
`commit_batch_output.py` 校验这些字段后，机械生成 `章节/第{N}章_摘要.md`，保留现有消费者需要的“概要、关键事件、情节点、基调、章尾钩子、证据”标签；同一批全部逐章紧凑事实还会与跨章观察一起写进 `_analysis_cache/批次-{起章}-{止章}.md`，供全局合并整批读取。模型不再输出章节卡表、逐章事件清单或多条情节点，也不为兼容文件再次读取原文。

### 批次剧情点

`raw-original` 输出 `_analysis_cache/批次-{起章}-{止章}.md`；`existing-results` 输出 `_analysis_cache/复用提取-{起章}-{止章}.md`。内容共用下列结构：
...
`existing-results` 的证据只指向运行计划列出的旧成果文件。输出用 `<!-- REUSED_CHAPTERS:{起章}-{止章} -->` 声明覆盖，只写批次剧情点及五类关联观察，不生成 `CHAPTER_START/END`，不覆盖旧逐章文件。旧资料没有的字段写“旧资料未提供”。

每个成功批次由 `commit_batch_output.py` 写入提交凭据。`raw-original` 收据含原文 hash 和逐章投影 hash；`existing-results` 收据只含旧成果二次提取缓存，不产生逐章输出。

**Stage 2 输出自检**：输入模式与 `run-plan.json` 一致｜原文章节块连续、不重叠且不超过10章/25,000字符｜已有成果模式没有读取原文｜每章只有紧凑字段｜剧情点包含因果与得失｜只保留关键事件/披露候选｜三维节奏分别解释｜跨批状态可继续。
## Stage 2 角色过滤
...
除前三强外只保留：机制ID｜名称｜用途｜适用章节/关系｜证据引用。不要复制完整卡片。

## 最强三个可复现模块卡

只展开全书最强的 3 个；不足 3 个时按实际数量写，不用弱机制凑数。
...
同一对角色的最新状态和历史演变都保留；“甲保护乙”与“乙依赖甲”分别记录。以此文件为唯一关系事实源，生成 `人物关系图/核心人物关系.png` 和 `人物关系图/关键关系演变.png`，并在 `拆文报告.md` 中嵌入。无法可靠渲染时记录原因，不编造图片。
...
- **结构分析**：主线 / 副线 / 剧情单元 / 覆盖率
- **人物动力与因果推进**：摘要剧情单元的起始目标、阻碍、关键变化来源、局面变化、得失和后续影响
- **人物关系及演变**：嵌入两张关系图，摘要关系方向、触发、双方得失和阶段变化
- **故事时间线与披露时间线**：引用 `剧情/节奏.md` 的 `EV-*` / `RV-*`，说明读者与角色的信息差及判断变化
- **信息披露与篇幅安排总览**：按剧情点汇总关键剧情信息如何被披露和展开（详情以 `_analysis_cache/批次-*.md`、`_analysis_cache/复用提取-*.md` 与 `剧情/节奏.md` 为准）
- **事件/情绪/篇幅三维节奏**：摘要 `剧情/节奏.md` 的 `AX-*`，说明三轴差异、展开/压缩/省略与关键区间
...
- **可复现模块**：列出 `剧情/情绪模块.md` Top 3 模块的读者期待、必要铺垫、成立条件、替换项、失效情形和不可照搬项
- **写法技巧**：1.{技巧名+用法} 2.{...}——覆盖一笔两用、延迟揭示、视角欺骗、对比锚点、行为循环、身体反应替代心理描写、**跨章回扣**（物品/意象在不同章节承担不同功能）
- **不建议模仿**：1.{问题+原因}
- **证据边界与待核事项**：原文缺口、失败批次、B/C 级推断、无法生成的图片和仍需定点核证的结论

报告只综合上述权威底层结果，不为写总报告重新阅读全文，也不复制所有逐章摘要。事实出现一次，其他位置通过剧情单元 ID、`EV/RV/AX/EM` ID、章节和定位引用。

### #387 最小消费契约

- 单书案例以 `剧情/*.md` 的剧情单元为准，可迁移机制以 `剧情/情绪模块.md` 的 `EM-*` 为准；主报告是阅读入口，不作为第二份结构化真源。
- 客观事件、披露节点和三维节奏分别使用 `EV-*`、`RV-*`、`AX-*`；披露必须引用事件，报告与机制卡通过 ID 引用，避免跨层复制全文。
- 每条可检索结论保留剧情单元/章节、`source_locator` 或定位词及证据强度；`chapter_index.csv` 的 `source_sha256` 和批次 receipt 用于判断来源是否变化。
- 下游先运行兼容检查器读取 `classification`、`current_capabilities`、`conflicts` 和覆盖范围。缺项时请求 #386 定点增强；不得把旧资料概括成伪造的当前主产物，也不得为入库重新阅读全文。
- 是否进入灵感库、跨书归并、差异保留和检索策略由 #387 决定；#386 不自动入库。
